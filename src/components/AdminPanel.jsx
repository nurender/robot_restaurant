...
const AdminPanel = () => {
  ...
  const [newNode, setNewNode] = useState({
    ...
    ai_enabled: true,
    ai_greeting: 'Welcome to Cyber Chef! Kya khilayein?',
  ...
  const addToManualOrder = item => {
    ...
    let newItems;
    if (existing) {
      newItems = manualOrderData.items.map(i => i.id === item.id ? {
        ...i,
        qty: i.qty + 1
      } : i);
    } else {
      newItems = [...manualOrderData.items, {
        ...item,
        qty: 1
      }];
    }
  ...
  const submitManualOrder = async () => {
    ...
    try {
      ...
      toast(isEditingOrder ? "Failed to update order" : "Failed to place order");
    }
  };
  ...
  const handleOrderUpdate = async orderId => {
    try {
      ...
      setOrders(prev => prev.map(o => o.id === orderId ? {
        ...
        customerPhone: editFormData.phone,
        items: editFormData.items
      } : o));
  ...
  const toggleDishActive = async item => {
    ...
    try {
      await apiService.updateDish(item.id, {
        ...item,
  ...
  const handleSaveRider = async () => {
    try {
      if (editingRiderId) {
        await apiService.updateRider(editingRiderId, {
          ...newRider
        });
      } else {
      ...
      toast(editingRiderId ? "Failed to update rider" : "Failed to recruit rider");
    }
  };
  ...
  if (orderedSidebar && orderedSidebar.length > 0 && !validPaths.includes(activeTab)) {
    return <div className="admin-layout ext-cls-42d9d60a">
  ...
  return <div className="admin-layout animate-fade-in">
      {isLoading && <div className="ext-cls-0fabc89b">
      ...
      `}</style>
      <AdminSidebar activeTab={activeTab} setActiveTab={handleTabChange} adminUser={adminUser} onLogout={handleLogout} isCollapsed={isSidebarCollapsed} setIsCollapsed={setIsSidebarCollapsed} orderedSidebar={orderedSidebar} />
      ...
      <main className={`admin-main ${isSidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
        <header className="main-header">
          ...
          <div className="header-profile-premium">
            ...
            if (activeWaiterCalls.length > 0) setActiveWaiterCalls([]);
          }}>
              <Bell size={22} color={activeWaiterCalls.length > 0 ? '#ef4444' : 'var(--text-muted)'} className={activeWaiterCalls.length > 0 ? 'pulse' : ''} />
              {activeWaiterCalls.length > 0 && <span className="ext-cls-b5652ddf">
                  {activeWaiterCalls.length}
                </span>}
              {activeWaiterCalls.length > 0 && <div className="animate-fade-in ext-cls-624612c0">
                  <div className="ext-cls-ca2f2f26">SERVICE ALERTS</div>
                  {activeWaiterCalls.map((c, i) => <div key={i} className="ext-cls-2eed4490">
                      <span className="ext-cls-c5e63a58">Table {c.table_number}</span>
                      <span className="ext-cls-2388e344">{new Date(c.time).toLocaleTimeString([], {
                    hour: '2-digit',
                    minute: '2-digit'
                  })}</span>
                    </div>)}
            ...
            <div className="profile-avatar-glow ex-style-131965" onClick={() => setShowProfileDropdown(!showProfileDropdown)}>
              {adminUser.name?.charAt(0)}
            </div>

            {showProfileDropdown && <div className="ex-style-8fb333">
                <div className="ex-style-cb10ec">
                  <div className="ex-style-abdd45">{adminUser.name}</div>
                  <div className="ex-style-f26bae">{adminUser.role === 'super_admin' ? 'Master Intelligence' : 'Branch Node'}</div>
                </div>
                <button onClick={handleLogout} className="ex-style-f01551">
                  <LogOut size={12} />
                  Sign Out
                </button>
              </div>}
        ...
        <div className="content-scrollable scrollbar-hidden">
          ...
          {activeTab === 'menu' && <NeuralInventoryView menuItems={menuItems} categories={categories} selectedCategory={selectedCategory} setSelectedCategory={setSelectedCategory} searchTerm={searchTerm} setSearchTerm={setSearchTerm} collapsedCats={collapsedCats} setCollapsedCats={setCollapsedCats} fetchData={fetchData} setShowCatPopup={setShowCatPopup} setNewDish={setNewDish} setEditingDishId={setEditingDishId} setFormError={setFormError} setShowMenuPopup={setShowMenuPopup} toggleDishActive={toggleDishActive} deleteDish={deleteDish} loadingStates={loadingStates} />}
          ...
          {activeTab === 'kitchen' && <KitchenSystemView kitchenOrders={kitchenOrders} fetchData={fetchData} isLoading={isLoading} editingOrderId={editingOrderId} setEditingOrderId={setEditingOrderId} editFormData={editFormData} setEditFormData={setEditFormData} handleOrderUpdate={handleOrderUpdate} kitchenItemChecked={kitchenItemChecked} setKitchenItemChecked={setKitchenItemChecked} updateOrderStatus={updateOrderStatus} />}

          {activeTab === 'coupons' && <CouponsView coupons={coupons} setShowCouponPopup={setShowCouponPopup} handleEditCoupon={handleEditCoupon} deleteCoupon={deleteCoupon} toggleCouponStatus={toggleCouponStatus} />}



          {activeTab === 'inventory' && <div className="view-container animate-slide-up ext-cls-40bdd684">
          ...
          {activeTab === 'rider_fleet' && <RiderFleetView riders={riders} setEditingRiderId={setEditingRiderId} setNewRider={setNewRider} setShowRiderPopup={setShowRiderPopup} fetchData={fetchData} handleEditRider={handleEditRider} handleDeleteRider={handleDeleteRider} />}

          {activeTab === 'reports' && <ReportsView orders={orders} menuItems={menuItems} categories={categories} restaurantsList={restaurantsList} customers={customers} feedbackList={feedbackList} fetchData={fetchData} formatDate={formatDate} />}

          {activeTab === 'qr_codes' && <QrCodesView restaurantTables={restaurantTables} adminUser={adminUser} fetchData={fetchData} printTableQR={printTableQR} />}

          {activeTab === 'qr_config' && <QrConfigView adminUser={adminUser} />}
          {activeTab === 'roles' && <RolesManagementView dbRoles={dbRoles} setDbRoles={setDbRoles} currentRoleData={currentRoleData} setCurrentRoleData={setCurrentRoleData} isRoleModalOpen={isRoleModalOpen} setIsRoleModalOpen={setIsRoleModalOpen} orderedSidebar={orderedSidebar} />}

          {activeTab === 'sidebar_order' && <SidebarConfiguratorView orderedSidebar={orderedSidebar} setOrderedSidebar={setOrderedSidebar} dragItemIndex={dragItemIndex} setDragItemIndex={setDragItemIndex} />}

          {activeTab === 'feedback' && <FeedbackView feedbackList={feedbackList} />}

          {activeTab === 'settings' && <SystemSettingsView adminUser={adminUser} restaurantsList={restaurantsList} />}

          {activeTab === 'hq' && <SuperAdminView adminUser={adminUser} />}
        </div>
      </main>
      {}
      <MenuEditorModal isOpen={showMenuPopup} onClose={() => setShowMenuPopup(false)} editingDishId={editingDishId} newDish={newDish} setNewDish={setNewDish} categories={categories} uploading={uploading} setUploading={setUploading} formError={formError} handleSaveDish={handleSaveDish} isSaving={loadingStates['save_dish']} />

      {}
      <MenuImportReviewModal isOpen={showImportReview} onClose={() => setShowImportReview(false)} extractedReviewData={extractedReviewData} setExtractedReviewData={setExtractedReviewData} adminUser={adminUser} categories={categories} fetchData={fetchData} />
    ...
    }} editingCouponId={editingCouponId} newCoupon={newCoupon} setNewCoupon={setNewCoupon} generateCouponCode={generateCouponCode} isLoading={loadingStates['save_coupon']} handleSaveCoupon={async () => {
      ...
      try {
        ...
        setActionLoading('save_coupon', false);
      }
    }} />

      {}
      <RiderModal isOpen={showRiderPopup} onClose={() => setShowRiderPopup(false)} editingRiderId={editingRiderId} newRider={newRider} setNewRider={setNewRider} isLoading={loadingStates['save_rider']} handleSaveRider={async () => {
      setActionLoading('save_rider', true);
      try {
        await handleSaveRider();
      } finally {
        setActionLoading('save_rider', false);
      }
    }} />
      {}
      <ManualOrderModal isOpen={showManualOrderPopup} onClose={() => setShowManualOrderPopup(false)} isEditingOrder={isEditingOrder} setIsEditingOrder={setIsEditingOrder} manualOrderData={manualOrderData} setManualOrderData={setManualOrderData} manualOrderSearch={manualOrderSearch} setManualOrderSearch={setManualOrderSearch} manualOrderCategory={manualOrderCategory} setManualOrderCategory={setManualOrderCategory} menuItems={menuItems} categories={categories} restaurantTables={restaurantTables} addToManualOrder={addToManualOrder} updateManualQty={updateManualQty} submitManualOrder={submitManualOrder} />
    </div>;
};
