...
import { Plus, Trash2, Edit2, Play, ChefHat, Save, Package, Search } from 'lucide-react';
import { API_URL } from '../config';

const CombosManager = ({ adminUser, restaurantId }) => {
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            // We fetch all menu items. Let's assume combos are also returned by /api/menu
            const res = await axios.get(`${API_URL}/api/menu${restaurantId ? `?restaurant_id=${restaurantId}` : ''}`);
            const allItems = res.data.data || res.data || [];
            
            // Separate combos from regular menu items
            const comboList = allItems.filter(i => i.is_combo === true);
            const regularList = allItems.filter(i => i.is_combo !== true);
            setCombos(comboList);
            setMenuItems(regularList);
        } catch (e) {
